...
## 🏗️ Hierarchia i Architektura Frameworka
...
*Decyduje o tym, GDZIE i JAK uruchamiamy testy.*
...
*Specjaliści od zadań technicznych. "Mięśnie" frameworka.*

**| Plik | Odpowiedzialność | Zależności | Użycie |**
| `WebActions.ts` | Pancerne akcje UI (click, fill), obsługa PDF i Excel. | `playwright`, `fs`, `exceljs` | Wstrzykiwany do `BasePage` |
...
*Standardy dla wszystkich obiektów stron i komponentów.*
...
### 🔌 4. Warstwa Fixtur i Page Objects (Logika Biznesowa)

*Odwzorowanie aplikacji i automatyzacja wstrzykiwania zależności.*
...
| `LoginPage.ts` | Mapowanie selektorów strony logowania na akcje. | Używa `this.actions` do interakcji. |
